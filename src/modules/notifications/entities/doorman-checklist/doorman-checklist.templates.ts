/**
 * 🚪 TEMPLATES DE NOTIFICAÇÃO - DOORMAN CHECKLIST
 * 
 * Templates específicos para checklist de porteiro.
...
/**
 * 🚪 TEMPLATES PARA DOORMAN CHECKLIST
 */
export const DOORMAN_CHECKLIST_TEMPLATES: Record<string, NotificationTemplate> = {
  created: {
    title: "Novo Checklist de Porteiro",
    message: "{userName} criou checklist de porteiro{postName} às {time}",
    priority: "NORMAL",
    recipients: "ACTIVE_SUPERVISORS_AND_ADMINS" // Supervisores ativos + admins
  },
  updated: {
    title: "Checklist de Porteiro Atualizado",
    message: "{userName} atualizou checklist de porteiro{postName} às {time}",
    priority: "NORMAL",
    recipients: "ACTIVE_SUPERVISORS_AND_ADMINS" // Supervisores ativos + admins
  },
  completed: {
    title: "Checklist de Porteiro Concluído",
    message: "{userName} concluiu checklist de porteiro{postName} às {time}",
    priority: "NORMAL",
    recipients: "ACTIVE_SUPERVISORS_AND_ADMINS" // Supervisores ativos + admins
  }
};
...
/**
 * 🔧 DOORMAN CHECKLIST TEMPLATE SERVICE
 */
export class DoormanChecklistTemplateService {
  
  /**
   * Obtém template por operação
   */
  static getTemplate(operation: string): NotificationTemplate | null {
    const template = DOORMAN_CHECKLIST_TEMPLATES[operation];
    if (!template) {
      console.warn(`Template não encontrado para operação: doormanChecklist.${operation}`);
      return null;
    }
    return template;
  }

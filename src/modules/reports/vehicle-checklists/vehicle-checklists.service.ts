...
> {
  ...
  protected async depoisDeCriar(data: any): Promise<void> {
    try {
      ...
      if (data.vehicleId && data.finalKm) {
        await this.atualizarKmDoVeiculo(data.vehicleId, data.finalKm);
      }
      ...
      console.error('Erro ao processar checklist de veículo criado:', error);
  ...
  protected async depoisDeAtualizar(
    ...
    try {
      // Buscar o checklist atualizado para obter vehicleId e finalKm
      ...
      // Atualizar quilometragem atual do veículo se finalKm foi informado
      if (checklistData?.vehicleId && checklistData?.finalKm) {
        await this.atualizarKmDoVeiculo(checklistData.vehicleId, checklistData.finalKm);
      }
      ...
      console.error(
        'Erro ao processar checklist de veículo atualizado:',
        error,
      );
      // Não falhar a operação principal por causa da notificação ou atualização de KM
    }
   ...
   */
  private async atualizarKmDoVeiculo(vehicleId: string | undefined, finalKm: number | undefined): Promise<void> {
    // Só atualiza se ambos os valores estiverem presentes
    if (!vehicleId || !finalKm || finalKm <= 0) {
      return;
    }
    ...
    try {
      ...
      console.log(`✅ Quilometragem atualizada para veículo ${vehicleId}: ${finalKm} km`);
    } catch (error) {
